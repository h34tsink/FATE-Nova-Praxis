---
aliases:
  - Kallius
tags:
  - pc
  - character
state: Sleeved
house: Kimura
skill-distribution: Specialist
refresh: 5
fate-points: 5
rep-rating: 3
high-concept: Black Market Memory Merchant
physical-stress: 6
mental-stress: 4
system-stress: 6
consequences: "Mental Moderate: Anxiety Spiral"
---

# Kallius

**Player:** James
**State:** [[Glossary/Cybersleeve|Sleeved]] (Delta Cybersleeve)
**House:** [[Factions/Houses/House Kimura|House Kimura]]
**Skill Distribution:** Specialist
**Refresh:** 5
**Fate Points:** 5
**Rep Rating:** 10
**Rep Bumps:** 10

---

## Background

Kallius is a black market memory merchant — a mnemonic savant who treats the past as currency and himself as the bank. Operating in [[Factions/Houses/House Kimura|House Kimura's]] shadow network, he buys, sells, and manipulates memories using illegal mnemonic editing techniques that would land him in a Coalition prison if discovered.

He carries a dangerous secret — something the Coalition would kill to bury — and the weight of it drives both his paranoia and his obsessive pursuit of neural perfection. Despite inhabiting a heavily augmented Delta Cybersleeve, part of him longs for his Pure self, the unaugmented body he left behind. That Alpha Biosleeve sits in storage like a conscience he can't quite discard.

His downtime is spent researching ways to halt ego fragmentation — a threat that's becoming personal after being possessed by the rogue S.I.M. "Death" in Session 5.

---

## Aspects

| # | Aspect | Type | Invoke | Compel |
| --- | --- | --- | --- | --- |
| **HC** | **Black Market Memory Merchant** | High Concept | Mnemonic ops, memory trade, underworld contacts | Authorities investigate; clients betray him; illegal work exposed |
| 2 | Perfect Neural Resurrection Through Data Mastery | Ambition | Ego preservation, resleeving expertise, fragmentation repair | Obsessive perfectionism; risks everything for a breakthrough |
| 3 | The Past Is Currency, I'm the Bank | Belief | Leverage through knowledge; reading people's histories | Transactional worldview alienates allies; treats relationships as trades |
| 4 | My Pure Self Still Calls | Connection | Conviction in moments of identity crisis; anchor to original self | Hesitates when augmentation is the answer; guilt over what he's become |
| 5 | I Know a Truth the Coalition Would Kill to Bury | Disadvantage | Dangerous leverage against powerful enemies | Paranoia; targeted by factions; the truth puts everyone near him at risk |
| 6 | Mnemonic Savant, Memory Palace Architect | Expertise | Memory editing, neural architecture, data recovery | Overestimates his control; ego fragmentation from overuse |
| 7 | Virtual or Not, Reality Is the Same to Me | Savant | *(Savant Disadvantage)* Seamless VR/meatspace operation; no lag hesitation | Blurred perception of real vs. virtual; exploitable by illusions/AR attacks |

---

## Skills

| Rank | Skills |
|------|--------|
| +5 Superb | [[Rules and Mechanics/Skills#Mnemonics\|Mnemonics]] |
| +4 Great | [[Rules and Mechanics/Skills#Engineering, Software (S.Eng)\|Engineering Software]], [[Rules and Mechanics/Skills#Engineering, Hardware (H.Eng)\|Engineering Hardware]], [[Rules and Mechanics/Skills#SINC\|SINC]] |
| +3 Good | [[Rules and Mechanics/Skills#Cohesion\|Cohesion]], [[Rules and Mechanics/Skills#Firearms\|Firearms]] |
| +2 Fair | [[Rules and Mechanics/Skills#Guile\|Guile]], [[Rules and Mechanics/Skills#Resolve\|Resolve]], [[Rules and Mechanics/Skills#Education\|Education]] |
| +1 Average | [[Rules and Mechanics/Skills#Networking, House (H.Net)\|House Networking]], [[Rules and Mechanics/Skills#Pilot\|Pilot]], [[Rules and Mechanics/Skills#Melee\|Melee]], [[Rules and Mechanics/Skills#Assets\|Assets]], [[Rules and Mechanics/Skills#Research\|Research]], [[Rules and Mechanics/Skills#Strategy\|Strategy]] |

> Specialist column shape: 1/2/2/3/6. SINC is a Special Skill — base rank 3 (Savant 1 + 2 milestone swaps), Skilled +1 = effective +4. Diplomacy dropped to +0 (unranked) from Expert conversion.

**Physical Skills:**

| Skill | Rank | Notes |
|-------|------|-------|
| [[Rules and Mechanics/Skills#Endurance\|Endurance]] | +4 | Delta Cybersleeve's primary stat |
| [[Rules and Mechanics/Skills#Athletics\|Athletics]] | +3 | |
| [[Rules and Mechanics/Skills#Perception\|Perception]] | +3 | |

> 10 Physical Skill Ranks (Delta Cybersleeve allocation)

---

## Stunts

| Stunt | Effect |
|-------|--------|
| [[Rules and Mechanics/Stunts#Savant\|Savant]] | Grants SINC 1, access to [[Rules and Mechanics/Savant Programs - Game Reference\|Savant Programs]], mandatory Disadvantage aspect |
| [[Rules and Mechanics/Stunts#Specialized Program\|Specialized Program]] | +2 bonus to execute Memory Hack |
| [[Rules and Mechanics/Stunts#Skilled\|Skilled]] | +1 to SINC (base 3 + Skilled = effective +4) |
| [[Rules and Mechanics/Stunts#Licensed\|Licensed]] | Can purchase restricted equipment using Rep-Rating instead of Assets |

---

## Savant Programs

| Program | Effect |
|---------|--------|
| [[Rules and Mechanics/Savant Programs - Game Reference#Memory Hack\|Memory Hack]] *(Specialized)* | Edit, implant, or extract memories. **+2 bonus** from Specialized Program stunt |
| [[Rules and Mechanics/Savant Programs - Game Reference#Access Memory\|Access Memory]] | Read a target's surface memories or recent experiences |

---

## Sleeves

> **Resleeving Note:** Kallius owns two sleeves. When he decants into the stored Alpha Biosleeve, his Physical Skills, sleeve aspects, augmentations, AR, and Physical Stress all change to match that sleeve. Mental skills, Savant programs, and non-physical stats stay the same. See each sleeve block below for the exact swap values.
>
> | Stat | Delta Cybersleeve (Active) | Alpha Biosleeve (Stored) |
> | --- | --- | --- |
> | **Physical Skills** | End +4, Ath +3, Per +3 (10) | Ath +3, End +3, Per +2 (8) |
> | **Physical Stress** | 6 (2 + End 4) | 5 (2 + End 3) |
> | **Sleeve Aspects** | Synthetic Biology, Inhuman Appearance | Gene-Fixed Biosleeve, Unattractive |
> | **Base AR** | 2 | 0 |
> | **Aug Points** | 7 | 2 |

### Alpha Biosleeve (Stored)

| Field | Value |
|-------|-------|
| **Type** | Biosleeve (Alpha), Cost 6 |
| **Aspects** | Gene-Fixed Biosleeve, Unattractive |
| **Physical Skills** | Athletics +3, Endurance +3, Perception +2 (8 ranks) |
| **Aug Points** | 2 |
| **Augmentations** | [[Rules and Mechanics/Augmentations#Arterial Toxin Filter\|Arterial Toxin Filter]], [[Rules and Mechanics/Augmentations#Exceptional Genegineering\|Exceptional Genegineering]] |
| **Condition** | Pristine |

> Kallius' original body. Represents "My Pure Self Still Calls."

### Delta Cybersleeve (Active)

| Field | Value |
|-------|-------|
| **Type** | Cybersleeve (Delta), Cost 10 |
| **Aspects** | Synthetic Biology, Inhuman Appearance |
| **Physical Skills** | Endurance +4, Athletics +3, Perception +3 (10 ranks) |
| **Aug Points** | 7 |
| **Base AR** | 2 |
| **Included Augs** | Apotheosis, Fully Synthetic, Optimized Power System |
| **Condition** | Pristine |

---

## Augmentations (Active Sleeve)

> **AP Accounting:** Delta Cybersleeve = 7 points. Bio Skin Sheath is a free sleeve choice (does not count). Data Jack is free with Apotheosis (does not count). All others cost 1 AP each.

| Augmentation | AV | Counts | Effect |
|-------------|-----|--------|--------|
| [[Rules and Mechanics/Augmentations#Bio Skin Sheath\|Bio Skin Sheath]] | — | Free (sleeve choice) | Natural skin covering; Synthetic Biology no longer Persistent |
| [[Rules and Mechanics/Augmentations#Data Jack\|Data Jack]] | — | Free (w/ Apotheosis) | Wired interface port + data storage |
| [[Rules and Mechanics/Augmentations#Smuggler's Compartment\|Smuggler's Compartment]] | 1 | ✓ | Hidden body compartment. **Upgrade: Charlatan** — fools radiation-based sensors |
| [[Rules and Mechanics/Augmentations#Custom Built\|Custom Built]] | 1 | ✓ | Sleeve built for specific task; gains descriptive aspect |
| [[Rules and Mechanics/Augmentations#Hidden Weapon System\|Hidden Weapon System]] | 1 | ✓ | Concealed weapon mount (restricted) |
| [[Rules and Mechanics/Augmentations#Retinal Implant\|Retinal Implant]] | 1 | ✓ | Zoom, low-light, AR interface |
| [[Rules and Mechanics/Augmentations#SIM Cell Module\|SIM Cell Module]] | 1 | ✓ | Hosts [[Characters/Players/Grace Pryzbylski/Grace Pryzbylski - FATE Character\|Grace Pryzbylski]] |
| [[Rules and Mechanics/Augmentations#SMR Transceiver\|SMR Transceiver]] | 1 | ✓ | Sub-mm radiation sensors; see through clothing, paper, plastic — not skin or metal |
| [[Rules and Mechanics/Augmentations#Crypto-Laser Link\|Crypto-Laser Link]] | 1 | ✓ | IR laser transceiver; silent, encrypted, invisible comms |

> **AP Used: 7 / 7** — sleeve full.

---

## Gear

| Item | Details |
|------|---------|
| [[Rules and Mechanics/Equipment#Credit Disk\|Credit Disk]] | Digital currency storage |
| Agent:Gamma — "Rose" | AI assistant; Eng Hardware +2, Eng Software +1, Mnemonics +1 |
| [[Rules and Mechanics/Equipment#Resurrection Insurance\|Resurrection Insurance:Delta]] | Sleeve restoration policy |
| PPC:Gamma — "Dorothy" | Portable computer; Education +2, Research +1, Insight +1 |
| [[Rules and Mechanics/Equipment#False CID\|False CID:Superior]] | High-authenticity fake identity |
| [[Rules and Mechanics/Melee Weapons#Short Sword, Mono-Edged\|Mono-Edged Short Sword]] | WR 3, Mono-Edged aspect |
| [[Rules and Mechanics/Equipment#CRI\|CRI]] | Cyber-Repair Injection; emergency sleeve repair |

---

## Drones

### Mr. Peepers

| Field | Value |
|-------|-------|
| **Tier** | Gamma |
| **Aspects** | Synthetic Biology, Inhuman Appearance, Modular Recon Drone with Rifle Conversion, Swift and Agile |
| **Skills** | Stealth +4, Athletics +3, Perception +3, Resolve +3, Endurance +2, Melee +1, Strategy +1, Insight +1 |
| **Augmentations** | Fully Synthetic, Cosmetic Customization, [[Rules and Mechanics/Augmentations#Hidden Weapon System\|Hidden Weapon System]], [[Rules and Mechanics/Drones Overview#Drone-Specific Augmentations\|Tiny Size]], Enhanced Sensors, [[Rules and Mechanics/Drones Overview#Drone-Specific Augmentations\|Armed]] |
| **AR** | 3 (Endurance +1) |

---

## Stress Tracks

| Track | Boxes | Source |
|-------|-------|-------|
| Physical | 6 | 2 base + Endurance 4 |
| Mental | 4 | 2 base + Resolve 2 |
| System | 6 | 2 base + Eng Software 4 |

## Active Consequences

| Severity | Type | Aspect | Recovery |
|----------|------|--------|----------|
| Moderate | Mental | **Anxiety Spiral** | End of session |

---

## Mindset

- **Mnemonic Core:** Intact
- **Extranet:** Connected
- **Insurance:** Gamma (via Resurrection Insurance)
- **Counters:** Resleeves: 1

---

## Connections

- **[[Characters/Players/Grace Pryzbylski/Grace Pryzbylski - FATE Character|Grace Pryzbylski]]** — Lives in Kallius' SIM Cell; he arranged her psychiatric care and offered sanctuary. She serves as his digital ward and surveillance specialist.
- **[[Characters/Players/Dustin (Grift) Halloway/Dustin Halloway - FATE Character|Grift]]** — Fellow Kimura Savant; mutual respect between mnemonic and intrusion specialists.
- **[[Factions/Houses/House Kimura|House Kimura]]** — Employer; tolerates his black market work as long as it serves the House's interests.

---

## Session Highlights

- **Session 1:** Helped a high-reputation individual recover from severe fragmentation. Discovered his custom walker was stolen by Alaric "The Broker" Baines (Apostate). Tracked the mech theft to [[Locations/Parallax Station|Parallax Station]].
- **Session 5:** Possessed by rogue S.I.M. "Death" — forced to attack [[Characters/Players/Aeddarius Crucial/Aeddarius Crucial - FATE Character|Aeddarius]] before the team subdued him. The experience left the Anxiety Spiral consequence.
- **Downtime Goal:** Research/create a mechanism to halt ego fragmentation.

---

## GM Notes

- **Compel Targets:** Black Market Memory Merchant (authorities investigate), My Pure Self Still Calls (hesitation when augmentation is the answer), Coalition Truth (paranoia, targeted by factions)
- **Invoke Targets:** Memory Palace Architect (mnemonic ops), The Past Is Currency (leverage through knowledge), Virtual or Not (operates seamlessly in both spaces)
- **Key Tension:** The SIM Cell Module hosts Grace — if Kallius is taken out or his sleeve is destroyed, Grace loses her home.
- **Vulnerability:** Post-possession anxiety; fragmentation is becoming a personal threat, not just a professional concern.
- **Growth Arc:** Reconciling his augmented existence with his Pure origins; deciding what to do with the truth he's carrying.

## Milestones

| # | Type | Spent On | Notes |
| --- | --- | --- | --- |
| 1 | Minor | SINC swap (base 1→2) | Swapped with a +2 skill |
| 2 | Major | Access Memory program + SINC swap (base 2→3) | SINC reaches base 3, effective +4 with Skilled |
| 3 | Minor | Skill rearrangement | Specialist column adjustment |
| 4 | Major | Licensed stunt | Can buy restricted gear via Rep-Rating |

> **Budget:** 4 milestones total. 4 spent, 0 remaining.
> **All milestones spent.**
