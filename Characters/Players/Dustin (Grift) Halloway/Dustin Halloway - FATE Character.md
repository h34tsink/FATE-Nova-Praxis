---
aliases:
  - Grift
  - Dustin Halloway
  - Dustin "Grift" Halloway
tags:
  - pc
  - character
state: Sleeved
house: Kimura
skill-distribution: Expert
refresh: 5
fate-points: 5
rep-rating: 6
high-concept: Ghost in the System Seeks Identity
physical-stress: 5
mental-stress: 5
system-stress: 6
---

# Dustin "Grift" Halloway

**Player:** Rob
**State:** [[Glossary/Cybersleeve|Sleeved]] (Beta Cybersleeve)
**House:** [[Factions/Houses/House Kimura|House Kimura]]
**Skill Distribution:** Expert
**Refresh:** 5
**Fate Points:** 5
**Rep Rating:** 7
**Rep Bumps:** 1

---

## Background

Dustin "Grift" Halloway is a digital ghost who has never felt at home in his own skin — or anyone else's. A natural social engineer and systems infiltrator, Grift drifted through random jobs and petty corporate theft before finding his niche in the grey space between House Kimura's legitimate operations and its shadow network.

He has a complicated relationship with [[Characters/Named NPCs/Briar Blackwood|Briar Blackwood]], whose favors always come with strings attached. A food delivery gig that went sideways during a social engineering op left a mark — he still flinches when deliveries come up. Underneath the cynicism and digital locks, Grift pines for a normal life he's not sure ever existed.

---

## Aspects

| # | Aspect | Type | Invoke | Compel |
| --- | --- | --- | --- | --- |
| **HC** | **Ghost in the System Seeks Identity** | High Concept | Infiltration, social engineering, blending in | Rootless; identity crisis; doesn't belong anywhere |
| 2 | Truth Hunter in a World of Lies | Ambition | Investigation, uncovering secrets, seeing through deception | Can't let a mystery go; obsessive pursuit of truth endangers the mission |
| 3 | Corporate Loyalty Is Programmed Slavery | Belief | Resisting authority; rallying others against oppression | Refuses legitimate orders; alienates allies in power structures |
| 4 | Briar's Favors Always Come with Interest | Connection | Calling in underworld contacts; access to black market intel | Briar calls in debts at the worst moment; dragged into side jobs |
| 5 | Curiosity Kills More Than Cats | Disadvantage | Noticing details others miss; asking the right question | Walks into obvious traps; investigates when he should run |
| 6 | Digital Locks Are Just Suggestions | Expertise | Hacking, bypassing security, social engineering tech systems | Overconfident; underestimates physical/analog defenses |
| 7 | Craves Sensations My Digital Mind Can't Process | Savant | *(Savant Disadvantage)* Empathy with biological beings; understanding physical experience | Lured by physical temptations; distracted by sensory input; existential despair |

---

## Skills

| Rank | Skills |
| --- | --- |
| +4 Great | [[Rules and Mechanics/Skills#Networking, House (H.Net)\|House Networking]], [[Rules and Mechanics/Skills#Mnemonics\|Mnemonics]], [[Rules and Mechanics/Skills#Engineering, Software (S.Eng)\|Engineering Software]] |
| +3 Good | [[Rules and Mechanics/Skills#Education\|Education]], [[Rules and Mechanics/Skills#Cohesion\|Cohesion]], [[Rules and Mechanics/Skills#Guile\|Guile]], [[Rules and Mechanics/Skills#SINC\|SINC]] |
| +2 Fair | [[Rules and Mechanics/Skills#Research\|Research]], [[Rules and Mechanics/Skills#Engineering, Hardware (H.Eng)\|Engineering Hardware]] |
| +1 Average | [[Rules and Mechanics/Skills#Strategy\|Strategy]], [[Rules and Mechanics/Skills#Resolve\|Resolve]], [[Rules and Mechanics/Skills#Insight\|Insight]], [[Rules and Mechanics/Skills#Intimidation\|Intimidation]], [[Rules and Mechanics/Skills#Stealth\|Stealth]], [[Rules and Mechanics/Skills#Diplomacy\|Diplomacy]], [[Rules and Mechanics/Skills#Melee\|Melee]], [[Rules and Mechanics/Skills#Assets\|Assets]] |

**Physical Skills:**

| Skill | Rank | Notes |
| --- | --- | --- |
| [[Rules and Mechanics/Skills#Endurance\|Endurance]] | +3 | Primary physical stat |
| [[Rules and Mechanics/Skills#Athletics\|Athletics]] | +2 | |
| [[Rules and Mechanics/Skills#Perception\|Perception]] | +1 | |

---

## Stunts

| Stunt | Effect |
| --- | --- |
| [[Rules and Mechanics/Stunts#Savant\|Savant]] | Grants SINC 1, access to [[Rules and Mechanics/Savant Programs - Game Reference\|Savant Programs]], mandatory Disadvantage aspect |
| [[Rules and Mechanics/Stunts#Skilled\|Skilled]] | +1 to SINC (included in rank above) |
| [[Rules and Mechanics/Stunts#Geek Speak\|Geek Speak]] | Use Education instead of Diplomacy or Guile to convince someone of a scientific fact (true or not) |

---

## Savant Programs

| Program | Effect |
| --- | --- |
| [[Rules and Mechanics/Savant Programs - Game Reference#System Invasion\|System Invasion]] | Breach system security; opposed by target's firewall or SINC |
| [[Rules and Mechanics/Savant Programs - Game Reference#Pattern Scan\|Pattern Scan]] | Detect patterns in data, identify encrypted communications, spot anomalies |

---

## Sleeve

### Beta Cybersleeve

| Field | Value |
| --- | --- |
| **Type** | [[Rules and Mechanics/Sleeves#Cybersleeves\|Cybersleeve]] (Beta), Cost 6 |
| **Aspects** | Synthetic Biology, Inhuman Appearance |
| **Physical Ranks** | 8 |
| **Aug Points** | 4 |
| **Base AR** | 1 |

---

## Augmentations

| Augmentation | AV | Effect |
| --- | --- | --- |
| [[Rules and Mechanics/Augmentations#Retinal Implant\|Retinal Implant]] | 1 | Zoom, low-light vision, AR interface. **Upgrade: X-Ray Sight** (+2 AV) — see through ~1 ft of material |
| [[Rules and Mechanics/Augmentations#Skinweave Armor\|Skinweave Armor]] | 1 | +1 AR, stacks with worn armor |

> **Total AR:** 2 (Cybersleeve base 1 + Skinweave 1). Note: cybersleeve AR does not stack with worn armor, but Skinweave does.

---

## Gear

| Item | Details |
| --- | --- |
| PPC:Beta — "Nikon" | Portable computer; Education +1, Strategy +1 |
| Agent:Beta — "Vor" | AI assistant; Research +1, Insight +1 |
| [[Rules and Mechanics/Armor#Reinforced Clothing\|Reinforced Clothing]] | AR +1 (does not stack with cybersleeve base AR) |
| [[Rules and Mechanics/Melee Weapons#Gravitic Baton\|Gravitic Baton]] | WR 3, Gravitic aspect |
| [[Rules and Mechanics/Equipment#Plasma Torch\|Plasma Torch]] | Cutting/welding tool |
| [[Rules and Mechanics/Equipment#Broadcast Screen\|Broadcast Screen]] | Jams surveillance in area; -4 penalty to Perception for electronic sensors |
| [[Rules and Mechanics/Explosives#Smoke Grenade\|Smoke Grenade]] | Area concealment |

---

## Stress Tracks

| Track | Boxes | Source |
| --- | --- | --- |
| Physical | 5 | 2 base + Endurance 3 |
| Mental | 5 | 2 base + Cohesion 3 |
| System | 6 | 2 base + Eng Software 4 |

---

## Mindset

- **Mnemonic Core:** Intact
- **Extranet:** Connected
- **Counters:** Resleeves: 1

---

## Connections

- **[[Characters/Named NPCs/Briar Blackwood|Briar Blackwood]]** — Dangerous fixer and confidant. Contracted Grift to build a security simulation that matched Europa Station's layout. Favors are never free.
- **[[Characters/Players/Kallius/Kallius - FATE Character|Kallius]]** — Fellow Kimura operative and Savant; mutual respect between mnemonic and intrusion specialists.
- **[[Characters/Players/Grace Pryzbylski/Grace Pryzbylski - FATE Character|Grace]]** — Shares the party's tech-specialist role; complementary skills.

---

## Session Highlights

- **Session 1:** Visited by Briar Blackwood; contracted to build a security simulation program. Recognized unsettling similarities between the simulation parameters and Europa Station's actual security layout.
- **Session 5:** Successfully trapped the rogue S.I.M. "Death" inside the station mainframe using a custom firewall after she possessed Kallius.

---

## GM Notes

- **Compel Targets:** Curiosity Kills (walks into obvious traps), Briar's Favors (called in at worst moments), Corporate Slavery (refuses legitimate authority), Craves Sensations (lured by physical temptations)
- **Invoke Targets:** Digital Locks (hacking), Truth Hunter (investigation), Ghost in the System (infiltration/stealth)
- **Approach:** Grift prefers social engineering and system intrusion over combat. He'll talk his way in, hack what he can't talk through, and fight only as a last resort.
- **Growth Arc:** Finding something worth being loyal to; resolving identity crisis between digital existence and physical longing

**Next Milestone:** Minor
