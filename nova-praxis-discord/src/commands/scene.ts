...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const result = await streamApi(prompt, 'quality', undefined, async (text) => {
      await interaction.editReply({ embeds: gmResponseEmbed('Scene', text) }).catch(() => {});
    });
    const customId = cacheForShare('Scene', result.output, interaction.user.id);
    await interaction.editReply({ embeds: gmResponseEmbed('Scene', result.output), components: [shareButton(customId)] });
  } catch (err) {
